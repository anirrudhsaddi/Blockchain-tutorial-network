...
describe('Starting tests for TradeApp', function() {
  ...
  it('website title should be TradeApp', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('TradeApp');
    })
  });
  ...
  it('navbar-brand should be TradeApp',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('TradeApp');
    });
  });
